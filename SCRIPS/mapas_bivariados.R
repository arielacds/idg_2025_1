...
db_port     = 5432               # puerto de escucha (alución al puerto que se instala Postgres)
...
mapa_bivariado_etiquetas = ggplot() + 
  geom_sf(data = sf_mapa_bi, aes(fill = bi_class), color = NA, show.legend = FALSE) + # como colorear el mapa, relacionado a las clases que se generaron antes
  geom_sf(data = sf_comunas_santiago, fill = NA, color = 'black', size = 0.4) + # bordes comunales 
  geom_sf_text(data = sf_comunas_centroides, aes(label = nom_comuna), size = 2, fontface = 'bold') + # nombre de comunas 
