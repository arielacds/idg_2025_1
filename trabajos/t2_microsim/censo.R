...
# Cada registro de la CASEN representa 1 persona. ID, escolaridad, edad, sexo
# Seleccionar variables.. Se deben eliminar N.A
...
# para el trabajo de microsimulación, hay que cambiar el YPC por la variable que c/u escogio
...
casen$ypc = as.integer(unclass(casen$ypc)) #se modifica para el trabajo
casen$Comuna = as.integer(unclass(casen$Comuna))
casen$sexo = as.integer(unclass(casen$sexo))
casen$edad = as.integer(unclass(casen$edad))

##===========================================================================================================
## PARA MICRO SIMULAR EL CENSO Y LA CASEN NO PUEDEN TENER NA, EN EL CASO DE ESC TIENE NA. IMPUTAR ESC Y E6A 
## depende de las cantidades de NA, si son muchos vale la pena imputar, pero si son pocos solo eliminar. 
## revisar la correlación entre las variables a imputar 
## calibraremos una regresión lineal: esc = B*e6a

# Imputación lineal de esc base a e6a 
idx_na = which(is.na(casen$esc))
...
## cambiar ypc por la variable elegida
zonas_ypc = aggregate(
  ypc ~ zone,
  data = sim_df,
  FUN  = function(x) median(x, na.rm = TRUE)
