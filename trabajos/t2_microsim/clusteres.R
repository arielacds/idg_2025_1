...
# nro optimo de clusteres, mayor n de clusteres, esta mas ajustado. queremos ver el cambio de tendencia 
# donde existe el quiebre indica el num eficiente de clusteres que vamos a utilizar 
fviz_nbclust(mall_scaled, kmeans, method = "wss") +
  labs(title = "Método del codo", x = "Número de clusters (K)", y = "WSS")

set.seed(123)  # para reproducibilidad, semilla hace que el centroide este siempre en el mismo lugar
km = kmeans(mall_scaled, centers = 4, nstart = 25)
# 4 es mi numero optimo 

# Añadir los clusters al dataframe original
mall$cluster = as.factor(km$cluster)
# asignamos un num de cluster, tienen caracteristicas similares entre personas 
...
##### Analisis de los graficos,y compraracion entre los clusteres, y que medidas tomar (soluciones)
